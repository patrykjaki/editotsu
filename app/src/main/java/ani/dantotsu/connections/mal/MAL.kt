...
object MAL {
    ...
    /**
     * Defensive eligibility for MAL login UI. Nonblank client ID means a
     * credentialed build where login can proceed; blank means a local /
     * no-credential build where the action must be hidden or disabled
     * with a truthful explanation instead of a button that can only fail.
     */
    fun isLoginAvailable(clientId: String = this.clientId): Boolean =
        clientId.isNotBlank()
